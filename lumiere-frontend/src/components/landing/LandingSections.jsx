...
function SignupCard({ onDemo }) {
  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      message.success("Account created. Welcome to Lumiere Maison.");
      setForm({ name: "", email: "", password: "" });
    } catch (error) {
      message.error(error.response?.data?.detail || "Could not create the account right now.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <h3>Start building environments that you can actually feel.</h3>
      <p className="lm-body-copy">
        Start building environments that you can actually feel - not just imagine.
      </p>

      <label>
        Full name
        <input name="name" value={form.name} onChange={onChange} placeholder="Ava Morgan" required />
      </label>
      <label>
        Email
        <input name="email" type="email" value={form.email} onChange={onChange} placeholder="you@studio.com" required />
      </label>
      <label>
        Password
        <input
      ...
      <div className="lm-signup-actions">
        <MotionButton type="submit" className="lm-button lm-button-primary" disabled={loading}>
          {loading ? "Creating..." : "Start Now"}
        </MotionButton>
        <MotionButton type="button" className="lm-button lm-button-secondary" onClick={onDemo}>
          Try Live Demo
        </MotionButton>
      </div>
    </motion.form>
    ...
    <div className="lm-page" ref={shellRef}>
      ...
      <main className="lm-main">
        <section className="lm-hero">
          ...
          >
            <LandingScene pointer={pointer} progress={sceneProgress} mobile={mobile} />
