...
export default function FurniturePicker({
  ...
  // Preview state — which model is being hovered + which DOM element
  ...
  // Show preview after a short delay so fast mouse-overs don't flicker
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      {/* ── Model grid ──────────────────────────────────────────────────── */}
      <div style={{
        padding: 16, background: `${COLORS.background}CC`,
        borderRadius: 16, border: `1px solid ${COLORS.secondary}60`, minHeight: 120,
      }}>
        {loading && (
      ...
      {selectedItem && (
        <div style={{
          ...
          <div style={{ color: COLORS.text, fontSize: 13 }}>
            Selected:{' '}
            <strong style={{ color: COLORS.action }}>
...
function ModelCard({ model, onPlace, onHover, onLeave }) {
  ...
  // Category icon mapping — add more as needed
  const categoryIcons = {
    chairs: '🪑', sofas: '🛋️', tables: '🪞',
    beds: '🛏️', cupboards: '🗄️', lamps: '💡',
    uncategorized: '📦',
  };
  ...
  return (
    ...
    >
      ...
      {/* Model name */}
      <span style={{
        fontSize: 11, color: hovered ? COLORS.action : COLORS.text,
