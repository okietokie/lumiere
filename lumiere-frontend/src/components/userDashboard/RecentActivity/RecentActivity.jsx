...
const RecentActivity = ({ activities, loading, onOpenProject }) => {
  const formatTime = (ts) => {
    if (!ts) return '';
    const d = new Date(ts);
    ...
    <section className="activity" style={{ animationDelay: '180ms' }}>
      ...
      <div className="activity__list">
        ...
        {!loading && activities?.map((activity, i) => (
          <div
            key={activity.id || i}
            className={`activity__item${activity.id ? ' activity__item--clickable' : ''}`}
            style={{ animationDelay: `${i * 40}ms` }}
            onClick={() => activity.id && onOpenProject?.({ id: activity.id, name: activity.project_name || activity.target })}
            ...
            onKeyDown={(event) => {
              ...
              if (event.key === 'Enter' || event.key === ' ') {
                ...
                onOpenProject?.({ id: activity.id, name: activity.project_name || activity.target });
              }
            }}
          >
            <div className={`activity__icon activity__icon--${activity.type ?? 'edited'}`}>
              {ACTION_ICONS[activity.type] ?? ACTION_ICONS.edited}
            </div>
            <div className="activity__body">
              <p className="activity__text">
                <span className="activity__action">{activity.action}</span>
                {activity.target && <> <span className="activity__target">"{activity.target}"</span></>}
                {activity.project_name && <span className="activity__proj"> in {activity.project_name}</span>}
              </p>
              <span className="activity__time">{formatTime(activity.timestamp)}</span>
            </div>
            {activity.id && (
              <span className="activity__jump">
                Open
