...
export default function ProjectViewerPage() {
  ...
  const sceneViewerUrl = buildSceneViewerUrl(glbUrl, project?.name, shareUrl);
    ...
    <div style={{
      ...
      <div style={{ padding: '18px 18px 8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
        <div>
          ...
          <h1 style={{ margin: '8px 0 0', fontSize: 'clamp(1.6rem, 4vw, 2.6rem)' }}>{project?.name || 'Room design'}</h1>
        </div>
          ...
          {loading ? (
            ...
            <Canvas camera={{ position: [8, 5.5, 8], fov: 42 }} shadows>
              <Suspense fallback={null}>
                <SceneShell scene={project?.scene} />
              </Suspense>
